...
export class BrokerConfigController {
  ...
  @Put("update/:id")
  async updateConfig(@Param("id") id: string, @Body() body: Partial<{
    name: string;
    url: string;
    port: number;
    protocol: BrokerProtocol;
  ...
  // Publicar
  @Post("publish/:id")
  async publish(@Param("id") id: string, @Body() body: { topic: string; message: any }) {
    const result = await this.configService.publish(Number(id), body.topic, body.message);
    return { ok: result.success, message: result.message };
  }
